<template>
  <div class="home-box">
    ...
    <el-form :inline="true" :model="searchForm" v-show="showSearch">
      ...
      <el-form-item>
        <el-button type="primary" @click="searchEvent" v-auth="'/adminAuth/list'" style="margin-left: -16px">查询
        </el-button>
      </el-form-item>
    </el-form>

    <right-toolbar v-model:showSearch="showSearch" @queryTable="getListData" @onImportClick="handleImport"
    ...
    <el-card>
      <el-table :data="tableData" v-loading="loading" element-loading-text="加载中..." border>
        ...
        <el-table-column prop="address" label="操作" min-width="280">
          <template #default="{ row }">
            <el-button type="primary" size="small" v-auth="'/adminAuth/look'" @click="onShowClick(row)">查看
            </el-button>
            <el-button type="info" size="small" v-auth="'/adminAuth/role'" @click="onShowRoleClick(row)">角色
            </el-button>
            <el-button type="danger" size="small" v-auth="'/adminAuth/del'" @click="onRemoveClick(row)">删除
            </el-button>
    ...
    <!-- 用户导入对话框 -->
    <UploadExcel v-model="upload.open" :title="upload.upLoadTitle" :url="switchServerUrl() + '/public/uploadFile'"
      @onSuccess="onSuccess" @onDownTemplate="onDownTemplate">
    </UploadExcel>
